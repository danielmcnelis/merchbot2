...
export default {
	...
	async execute(interaction) {
        try {
            // if (!args.length) return interaction.reply({ content: `Please specify the card(s) you wish to buy.`})
            ...
            const price = shopSale ? Math.ceil(print.marketPrice * 1.1) : interaction.options.getNumber('price')
            ...
            if (!shopSale && price < shopBuybackPrice) return interaction.reply({ content: `You cannot buy a card for less ${stardust} than The Shop ${merchant} would pay for it.` })
            ...
            const filter = i => i.customId.startsWith('Buy-') && i.user.id === interaction.user.id;
