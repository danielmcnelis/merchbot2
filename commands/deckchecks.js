            ...
            if (isProgrammer(interaction.member)) {
                ...
                for (let i = 0; i < entries.length; i++) {
                    ...
                    if (zeroCopiesOwned?.length || oneCopyOwned?.length || twoCopiesOwned?.length) {
                        await interaction.channel.send({ content: `High Alert: <@${entry.player.discordId}> does not own every card in their deck. <a:high_alert:1384949884221919392>`}).catch((err) => console.log(err))
                    } else {
