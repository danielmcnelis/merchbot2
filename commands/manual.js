...
export default {
    ...
    async execute(interaction) {
        try {
            ...
            const losingMember = await interaction.guild?.members.fetch(losingUser.id).catch((err) => console.log(err))
            ...
            if (chipsWinner <= chipsLoser) chipsWinner = chipsLoser + 2
            if (chipsWinner > 8) chipsWinner = 8
            if (chipsLoser > 4) chipsLoser = 4
            ...
            const content = `A manual Arena ${arena} loss by <@${losingPlayer.discordId}> (+5<:starchips:1488939806053498931>)  to <@${winningPlayer.discordId}> (+8<:starchips:1488939806053498931>) has been recorded.`
