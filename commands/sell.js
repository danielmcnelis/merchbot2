...
import { calculateNewMarketPrice, getBuyerConfirmation, checkBinderForRemoval, checkWishlistForRemoval } from '../functions/transaction.js'
        ...
        try {
            ...
            try {
                ...
                if (confirmation.customId.includes('Yes') && shopSale) {
                    ...
                    return interaction.editReply({ content: `Not a problem. Nothing was sold to ${shopSale ? `The Shop ${merchant}` : buyer.name}.`, components: [] })
                }
            } catch (err) {
                console.log(err)
                return interaction.editReply({ content: `Sorry, time's up. Nothing was sold to ${shopSale ? `The Shop ${merchant}` : buyer.name}.`, components: [] });
            }                        
        } catch (err) {
            console.log(err)
        }
