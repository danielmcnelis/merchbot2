...
const { AOD, COC, CTP, FON, LDM, com, rar, sup, ult, scr, starchips, stardust } = emojis

export default {
	data: new SlashCommandBuilder()
		.setName('spec')
		.setDescription('Purchase a special edition box! 🎴')
        .addStringOption(str =>
            str
                .setName('set')
	...
	async execute(interaction) {
        try {     
            ...
            await interaction.reply({ content: `${player.name}, you have ${money}${eval(set.currency)}. Do you want to spend ${Math.round(set.specPrice * discount)}${eval(set.currency)} on an ${set.name} ${eval(set.emoji)} Special Edition box?`, components: [row] })
