...
const { gladiators, foxy, shrine, cavebob, AOD, COC, CTP, FON, LDM, WCR, king, arena, aqua, beast, destiny, fairy, fiend, flip, machine, monarch, pyro, reptile, rock, spellcaster, warrior, zombie, starchips, ult, mushrooms, bolts, orbs, shields, skulls, droplets, firecrackers, moais, beads, familiars, amulets, sandals, crowns, cacti, blackwing, feathers, dragon, gems, frog, lotuses, hero, capes, insect, nets, plants, roses } = emojis
...
export const startRound = async (arenaEntries) => {
    ...
    if (info.round === 7) {
        ...
        const title = `${arena} - ${blackwing}${dragon}${frog} -- Arena Round ${info.round} -- ${hero}${insect}${plant} - ${arena}` 
...
export const forfeit = async (winnerId, loserId) => {
		...
		channel.send({ content: `${losingEntry.playerName} (+0${starchips}), your Arena loss to ${winningEntry.playerName} (+4${starchips}) has been recorded.`})
