...
const { AOD, COC, CTP, FON, LDM, falling, rising, forgestone, gem, orb, swords, beast, blue, bronze, DRT, fiend, thunder, zombie, skull, familiar, battery, cactus, cavebob, checkmark, closed, com, credits, cultured, diamond, dinosaur, DOC, LPK, egg, emptybox, evil, FiC, fire, fish, god, gold, hook, koolaid, leatherbound, legend, lmfao, mad, master, merchant, milleye, moai, mushroom, no, open, ORF, TEB, warrior, spellcaster, dragon, plant, platinum, rar, red, reptile, rock, rocks, rose, sad, scr, silver, soldier, starchips, stardust, stare, dimmadome, stonks, sup, tix, ult, wokefrog, yellow, yes, ygocard } = emojis
...
export const applyPriceDecay = async () => {
    ...
    for (let i = 0; i < prints.length; i++) {
        ...
        if (shop_percent < 0.10) {
            ...
            console.log(`${print.cardCode} - ${print.cardName} decayed UP (shop owns ${shop_pop} out of ${total}, z_diff = ${z_diff}) from ${print.marketPrice} to ${print.marketPrice + (0.02 * currentPrice * z_diff)}`)
            ...
            console.log(`${print.cardCode} - ${print.cardName} decayed DOWN (shop owns ${shop_pop} out of ${total}, z_diff = ${z_diff}) from ${print.marketPrice} to ${print.marketPrice - (0.06 * currentPrice * z_diff)}`)
